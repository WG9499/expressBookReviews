...
let users = [{"username":"wathek", "password":"qwerty"}];
...
const userAuthenticated = (username,password)=>{ //returns boolean
//write code to check if username and password match the one we have in records.
    const corrAuth = users.filter((el)=> el.username === username && el.password === password);
...
regd_users.post("/login", (req,res) => {
 ...
 if (userAuthenticated(username,password)){
    ...
    return res.status(208).json({message : "Error logging in, check your credentials! "})
 }
